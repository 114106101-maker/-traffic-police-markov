...
def create_interactive_graph(n, edges_with_weights, steady_v=None):
    ...
    for i in range(1, n + 1):
        ...
        net.add_node(i, label=f"路口 {i}", color=color, title=f"長期機率: {steady_v[i-1]:.4f}" if steady_v is not None else "")
    for u, v, w in edges_with_weights:
        if 1 <= u <= n and 1 <= v <= n:
